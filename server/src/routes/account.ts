...
router.post('/update', httpOnlyAuthentication, async(request, response) => {
    imageUpload(request, response, async (error) => {
        ...
        if (!parameters || error) return response.sendStatus(400);  // information sent is incomplete
        const { uuid } = request.authenticated!;
        const { userCommonName, biography } = parameters;

        const profilePicture = (request.files as any).profile_picture ? (request.files as any).profile_picture[0] as Express.MulterS3.File : undefined;
        const bannerPicture = (request.files as any).banner_picture ? (request.files as any).banner_picture[0] as Express.MulterS3.File : undefined;
        ...
        if (profilePicture) await connection.query('UPDATE `user` SET user_profile_picture = ? WHERE user_id = UNHEX(?)', [ profilePicture.location, uuid ]);
        if (bannerPicture) await connection.query('UPDATE `user` SET user_banner_picture = ? WHERE user_id = UNHEX(?)', [ bannerPicture.location, uuid ]);
